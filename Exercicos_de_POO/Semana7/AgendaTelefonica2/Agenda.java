...
class Agenda {
    ...
    public void addContact(Contact contact) {
        ...
        if(findContact(contact.getName()) == null){
            Contact validContact = new Contact(contact.getName(), null);
            int j = 0;
            for (int i = 0; i < contact.getFones().size(); i++) { 
                validContact.addFone(contact.getFones().get(i));
                if(!contact.getFones().get(i).isValid()){
                    j++;
                }
            }
            if(j < contact.getFones().size()){
                this.contacts.add(validContact);
            } 
